...
Six places carry the PixelKit name in this template. Change them or you will ship an app that reports itself as someone else's:
...
- **Observability and provenance** — Every hook imported from `@pixelkit-labs/sdk` provides `source: 'hardware' | 'derived' | 'unavailable'`, traced operations with correlation IDs, and per-module error counts.
...
| :--- | :--- | :--- |
| `@pixelkit-labs/mlkit` | Gemini Nano, ML Kit vision or ML Kit language | Remove from `package.json`, and remove the AI Lab sections that use them |
| `@pixelkit-labs/native` | CPU, GPU, memory, thermals, torch, haptics detail, radios | Remove from `package.json`; the hooks then report `unavailable`, which is honest but empty |
| `scripts/hilight-daemon/` | The camera-bar LEDs | The `useHiLight` section in `SensorsLabScreen.tsx` |
| `@google/genai` | Cloud Gemini | Remove from `package.json` and the Cloud AI section |

After any deletion, run `npm run parity`. It will tell you exactly which map entries, screens and documentation entries you left behind — that is what it is for.

## Adding your own hook or section

1. Import your target hook from `@pixelkit-labs/sdk` (or `@pixelkit-labs/sdk/mlkit`).
2. Give it a home in `src/core/surface.ts` — a tab and a section.
3. Build the card or section that demonstrates it on that screen.
4. Document it or register its contract in `src/screens/docs/` or `data/hooks/`.
5. Run `npm run verify`. It validates TypeScript, documentation contracts, and surface parity.
