...
- `npm run verify` must pass: `check-deps` (ensures all `@pixelkit-labs` packages exist on npm and `package-lock.json` matches `package.json`), `typecheck` with 0 errors, then `parity` with no unhomed hooks and no unreachable documented actions.
